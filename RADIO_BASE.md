...
- **読点（、）は自然な息継ぎの位置にだけ**。文節ごとに細かく打たない。synth が「直近の区切りから14文字未満の細切れの読点」を自動で間引く（`synth.thin_commas`）ので打ちすぎても保険は効くが、もとから打ちすぎない方がよい（kota 指摘 2026-06-18「読点が多くて聞きづらい」）。
- ⚠️ 読点を間引くと**間が減り、同じ文字数でも音声は2〜3割みじかくなる**。9〜11分を狙うなら**カナ台本で約4000〜4800字**を目安に（従来3300〜3900字より厚め）。生成後に必ず長さを確認。
